            ...
            popularDishes.map((dish)=>{
              return(
                <div key={dish.id} className='flex items-center gap-4 bg-[#1f1f1f] rounded-[15px] px-6 py-4 mt-4 mx-6'>
                  <h1 className='text-[#f5f5f5] font-bold text-xl mr-3'>{dish.id < 10 ?`0${dish.id}`:dish.id}</h1>
                  ...
                  <div>
                    ...
                    <p className='text-[#f5f5f5] text-sm font-semibold mt-1'><span className='text-[#ababab]'>Orders: </span>{dish.numberOfOrders} 
                    </p>
                  </div>
